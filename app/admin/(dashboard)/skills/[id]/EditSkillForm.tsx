...
export default function EditSkillForm({ category }: { category: SkillCategory }) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<Omit<SkillCategory, '_id'>>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/skills/${category._id}`, {
        method: 'PUT',
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-stone-100">Edit Skill Category</h1>
            <p className="text-stone-400 mt-1">{category.title}</p>
          </div>
        </div>
        <DeleteButton id={category._id} endpoint="skills" itemName="Skill Category" redirectPath="/admin/skills" />
      </div>
